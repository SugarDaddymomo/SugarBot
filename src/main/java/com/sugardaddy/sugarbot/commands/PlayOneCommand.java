...
public class PlayOneCommand extends ListenerAdapter {

    Logger logger = LoggerFactory.getLogger(PlayOneCommand.class);
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        String restCommand = "";
        try {
            if (event.getMessage().getContentRaw().contains("$playone") && !event.getMessage().getAuthor().isBot()) {
...
//                }
                ...
                if (!isURL(restCommand)) {
                    ...
                    PlayerManager.getINSTANCE().loadAndPlaySingleTrack(event.getChannel().asTextChannel(), link);
                } else {
                    logger.info(restCommand);
                    PlayerManager.getINSTANCE().loadAndPlaySingleTrack(event.getChannel().asTextChannel(), restCommand);
                }
            }
        } catch (Exception e) {
